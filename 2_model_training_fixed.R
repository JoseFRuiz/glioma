# =============================================================================
# Glioma Survival Prediction - Model Training Script (FIXED)
# =============================================================================
...
# =============================================================================
# Model Training Functions (FIXED to match original)
# =============================================================================

setup_train_control <- function(method = "boot", number = 1, p = 0.7) {
  # Set up training control parameters (same as original rf_classifier.R)
  trainControl(
    method = method,
    number = number,
    p = p,
...
train_rf_model <- function(train_data, ctrl) {
  # Train Random Forest model (same as original)
...
calculate_calibration_parameters <- function(model, train_data) {
  # Calculate alpha and beta parameters for linear calibration (same as original)
  cat("Calculating calibration parameters...\n")
  
  # Get training indices from the model (this is the internal caret split)
  train_indices <- model$control$index[[1]]
  
  # Get predictions on training data (internal training set)
  ...
  cat("Calibration samples:", length(train_indices), "\n")
...
save_model_and_parameters <- function(model, calibration_params, output_dir = "results") {
  ...
  calib_df <- data.frame(
    Parameter = c("alpha", "beta", "calibration_r_squared", "calibration_samples"),
    Value = c(calibration_params$alpha, calibration_params$beta, 
              calibration_params$calibration_r_squared, length(calibration_params$calibration_indices))
  )
  ...
  cat("Total training samples:", nrow(model$trainingData), "\n")
  cat("Calibration samples:", length(calibration_params$calibration_indices), "\n")
...
main_model_training <- function() {
  ...
  cat("Glioma Survival Prediction - Model Training (FIXED)\n")
  cat("==================================================\n\n")
  ...
  cat("MODEL TRAINING COMPLETE (FIXED)\n")
