...
create_scatter_plot <- function(results, method, output_dir = "results") {
  ...
  pdf(paste0(output_dir, "/predicted_vs_actual_xcell_exp_log_", method, ".pdf"))
  par(mar = c(5, 4, 4, 6))
  
  plot(exp(results$actual), exp(results$predictions),
       xlab = "Actual Days to Death",
       ylab = "Predicted Days to Death",
       main = paste("Predicted vs Actual (xCell features, ", method, ")"),
       pch = 19, col = "blue")
  
  abline(0, 1, col = "red", lty = 2)
  
  text(min(results$actual), max(results$predictions),
       paste("Correlation:", round(results$correlation, 3), 
             "\nRMSE:", round(results$rmse, 2), "log(days)"),
